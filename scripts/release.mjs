#!/usr/bin/env node
/**
 * agile-docs 发版脚本（npm run release）
 *
 * 文档仓不发 npm：发布 = 部署 GitHub Pages + 创建 GitHub Release 归档，
 * 两者均由 tag 触发的 release workflow 执行。本地只负责质量门 → bump → tag → push。
 *
 * 用法：
  ...
  while (Date.now() - started < timeoutMs) {
    try {
      const res = await fetch(`${api}&created=>${new Date(started - 60_000).toISOString()}`, {
        headers: { 'User-Agent': 'agile-docs-release' },
      });
      if (res.ok) {
...
async function main() {
  ...
  // ---------- 3. 质量门（与 CI 相同）----------
  log(`▶ 质量门：docs:build（死链/语法检查）`);
  if (DRY_RUN) {
    log('（dry-run 跳过）');
  } else {
    await sh('pnpm', ['docs:build']);
  }

  // ---------- 4. 确认 ----------
  log(`▶ 发布 ${current} → ${next}（tag ${tag} → ${repo.owner}/${repo.repo}，部署 Pages + GitHub Release 归档）`);
  if (!YES) {
  ...
  if (conclusion === 'success') {
    log(`✔ 发布成功`);
    log(`  文档站：https://${repo.owner}.github.io/${repo.repo}/`);
