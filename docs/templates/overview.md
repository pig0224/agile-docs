...
项目模板统一登记在模板注册中心 [agile-templates](https://github.com/pig0224/agile-templates)——`registry.json` 声明全部模板，**新增模板无需升级 CLI**。CLI 从模板源读取注册中心，源地址可换成团队私有仓库。
...
AI 协助建项目（撞名核对 + 成员目录命名问答 + 项目约定问答定制 + 团队库匹配确认）用插件命令 `/agile:init`，见[插件命令详解](/plugin/commands)。
...
两类模板**地位一致**，生成后均**平铺落盘** `projects/<目录名>/`（无系统目录层级）：
...
组合模板把一组成员模板打包登记（registry.json `solutions` 数组），一次 `init project` 平铺生成全部成员项目。**成员是组合专属的完整模板骨架**（`solutions/<组合名>/<成员名>/`，不引用 singles）——可为同一职责域在不同组合里做差异化定制（规范骨架三文件、前端 `docs/ui.md` 照常带出）：
...
<span v-pre>`{{name}}`</span> 恒等于实际落地目录名——平铺目录名全局唯一，包名随之唯一。

## 缓存机制

模板仓库的本地缓存位于 `~/.agile/templates/<url哈希>`（本机所有 workspace 共用）：

- `template list` / `init project` 默认**读本地缓存**（不联网）；`agile template update` 或 `agile sync` 才联网刷新
