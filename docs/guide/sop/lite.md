...
**性质**（编号前缀）决定审批人，**深度**（完整 / 轻量）决定文档写多少：

| 形态 | 性质 / 审批人 | 典型场景 | 编号分配 | 入口 |
|---|---|---|---|---|
| STO 轻量 | 业务需求 / 产品一句话确认 | 独立提出的 mini feat、文案/样式调整 | 产品确认后分配 | `/agile:backend` / `/agile:frontend` |
| BUG | 缺陷 / 无需审批（回归正确） | 行为与预期不符的修复 | 报告人/负责人顺延 | `/agile:fix-bug`（标准入口） |
| OPS | 技术变更 / 运维审批 | 重构、依赖升级、日志/配置/CI 微调、组件库维护升级（`/agile:ui maintain`） | 运维顺延分配（前端发起时向运维申领编号） | `/agile:backend` / `/agile:frontend` |

> **「小」不是 OPS 的判定维度**——文案/样式调整业务可见，归 STO 轻量；OPS 只收纯技术项。

> **编号顺延参照系**：以 `process-docs/` 现有编号为准（全形态统一编号序列）；`/agile:prd` 自动分配另查产品知识库 `requirements/`（完整需求可能已立项未同步）。`/agile:sync-req` 对用户给的编号强制查重。
...
1. **分配新编号**——由有分配权者分配（STO = 产品）并**两处查重**（`process-docs/` 与产品知识库 `requirements/`）
...
3. worktree 照常创建、PR 照常提交（标题标 `[OPS-007]`）、CI 绿 → 负责人自查一行确认 → 合并

### 组件库维护（OPS）：`/agile:ui maintain`

1. `/agile:ui maintain 日期选择器增加范围快捷项`——命令默认引导走 OPS 轻量：`/agile:sync-req OPS-xxx <变更一句话>` 建目录，worktree / PR 照常执行
2. 升级 = 改实现 + 更新测试 + CHANGELOG 登记；废弃 = deprecated 标记 + 迁移指引
3. **改设计 token 须同步回写项目 `docs/ui.md`**（以此为准）；grep 组件库引用列出受影响页面与批量验证建议
...
2. 触发升级出口：停止轻量流程，大伟知会林悦；产品确认升级完整 STO 并分配新编号 **STO-014**（两处查重：`process-docs/` 现有最大 STO-013、产品知识库 `requirements/` 现有最大 STO-012 → STO-014 未占用）
