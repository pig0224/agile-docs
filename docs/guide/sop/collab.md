...
- **固化 e2e 脚本**入项目 `e2e/`（长期资产，随页面同 PR 演进，供 `/agile:run-test` 与 stage 冒烟复用；**浏览器行为 bug 的复现与回归也固化于此**——相对路径可移植，任何成员可跑；**默认不进 PR CI 门禁**——e2e 依赖浏览器与环境，不稳定且耗时较长，各项目可选择执行关键路径冒烟子集）
- **一次性临时脚本**（数据构造、接口重放、临时排查）放 `process-docs/<编号>/scripts/`，**严禁放置在 projects/ 下的项目内**——本机留存不入库；有长期回归价值的复现测试进项目测试套件或 `e2e/`
