...
- **本板块（SOP）**：定义「谁、在哪一步、做什么、产出什么」，是执行流程的依据
- **插件命令**（见[插件概览](/plugin/overview)）：`/agile:xxx` 命令按本 SOP 辅助执行各步骤；Agent 辅助执行，**验收结论由人做出**
...
- 不需要另设架构评审节点：小需求负责人自查设计；重大/跨端需求，开工前邀请对端与产品进行 15 分钟设计走查（会话级，结论记入 design.md，不走 PR）
...
- 在**外部平台**（腾讯文档 / 飞书等）编写需求文档（最低结构要求：背景、目标、验收标准 AC（≥ 1 条））——产品不直接操作工作区，把需求文档链接或内容交给负责人
...
### 测试 = 两个验收动作 + 汇总口径（无专职岗位，人员可兼任）

1. **交叉验收**：前端验后端接口、后端验前端页面
2. **AC 验收**：产品按 requirement.md 验收标准逐条确认（stage 环境）
3. 负责人将结论汇总进 review.md（`/agile:review` 辅助汇总与门禁判定）
4. **测试设计与执行**：测试案例设计（`/agile:gen-test`）由负责人执行（生命周期 #3，先于对端拉取环境）；`/agile:run-test` 若有独立测试人员则由其执行，没有则由开发兼任 + 产品 stage 验收兜底
...
| 页面 | 内容 |
|---|---|
| [初始化](/guide/sop/init) | 架构师一次性交付：workspace、外部资源、模板、插件、CI/CD |
