...
**组合模板**：一个组合一次生成全部成员项目，**平铺**落盘 `projects/<成员目录名>/`（无系统目录层级），输出汇报以组合名为标签。成员是**组合专属的完整模板骨架**（`solutions/<组合名>/<成员名>/`，不引用 singles），规范骨架三文件照常带出。**命名约束**：模板名 / 组合名 / 成员名三段全局唯一（成员平铺落盘后直接占用 `projects/` 顶层目录名），组合登记与成员目录双向一致。
