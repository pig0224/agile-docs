...
Agile 插件的 7 个角色。命令做「前置校验 → 汇总」；实施分两类——**调研产出类**（prd / architect / gen-test / ui）委派对应角色 subagent 执行，**执行循环类**（backend / frontend / run-test / fix-bug）主会话按角色规范（`roles/*.md`；run-test 用 `agents/test-engineer.md` 的 Stage 2 规范）直接执行，过程全程可见、可随时纠偏。
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 把模糊业务想法整理成结构化产品文档；为需求编号编写 PRD（/agile:prd 委派执行） |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 为需求做架构/状态机/接口设计；SDD 设计阶段（/agile:architect 委派执行） |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 后端开发任务（/agile:backend——主会话按本规范直接执行） |
| 输入 | design.md + 测试案例 + worktree 路径 + 任务清单（每批 2~3） |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 前端开发任务（/agile:frontend——主会话按本规范直接执行） |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 组件库建设/扩展、页面原型产出、组件升级废弃（/agile:ui 委派执行） |
| 输入 | UI 规范 + 交互规范 + 变更需求 |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 生成测试案例（/agile:gen-test，Stage 1——委派执行）；执行测试验收（/agile:run-test，Stage 2——主会话按本规范直接执行） |
...
| 项目 | 说明 |
|---|---|
| 何时使用 | 快速修复 bug（/agile:fix-bug——主会话按本规范直接执行）；定位需要根因分析的缺陷 |
| 输入 | bug 描述 + 涉及项目 + 任务编号 |
