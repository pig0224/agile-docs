...
| 产物 | `biz-product-docs/requirements/<编号>/`：`PRD.md`、`AC.md`、`feature-tree.md`、`menu-tree.md` |
...
| 产物 | 按 SKILL 附录 A 模板直接创建任务目录（初始 8 个 .md，幂等）。完整：PRD+AC 并入 `requirement.md`，功能树/菜单树复制（产品知识库原件保留）；轻量：`requirement.md` 落一句话需求 + 1–2 条 AC（头部轻量标记）、`gen-test.md` 填豁免行 |
...
设计约束：技术选型只能来自公司规范允许清单；复用优先（先查技术知识库既有设计）；每个接口/表可追溯 AC；不确定处显式标注 TBD。
...
| 前置 | design.md 已填充（红线）；开发环境准备：`agile worktree create feat/<编号>`；工作区干净 |
...
**硬规则**：AI 只记录、格式化与判定门禁，**不代替任何角色的验收**——交叉验收、AC 验收的结论必须由人做出。全部通过 → 可交付 PR；有未闭环 → 不得交付，缺陷走 `/agile:fix-bug` 修复后重新执行。轻量通道（STO 轻量 / BUG / OPS）降级为报告人/提需求人一行确认（[SOP · 轻量通道](/guide/sop/lite)）。
...
**硬规则**：本命令**不执行任何发布动作**——合并 PR、部署、发版一律人工执行，只做前置检查、记录起草与结果登记。发布顺序不变：合并先于发版。**回滚方案必备要素**：回滚目标、回滚步骤、数据影响与处理、回滚后验证点——按本次变更内容起草，不得简化为仅写「回滚上个版本」。轻量通道涉及部署时记一行（变更 + 回滚点）。
