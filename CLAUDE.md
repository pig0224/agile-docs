...
├── guide/                 # CLI 板块：getting-started / concepts / commands / troubleshooting
...
- **内容准确性高于一切**：命令参数表从 `../agile-cli/src/commands/*.ts` 的 commander 定义核对；插件命令细节从 `../agile-plugins/plugins/agile/` 核对——改文档先看代码，不凭记忆。
- **新增页面必须同步 config.ts 的 sidebar**，否则构建不报错但不可导航。
- 链接用 VitePress 相对路径（`/guide/xxx`）；`base: '/agile-docs/'` 已配置，无需手写前缀。
- 全站中文；代码块内命令输出保持原样。
- 兄弟仓库代码变更（命令/参数增删）时，同步更新 `guide/commands.md`——这是文档站最容易腐烂的一页。
